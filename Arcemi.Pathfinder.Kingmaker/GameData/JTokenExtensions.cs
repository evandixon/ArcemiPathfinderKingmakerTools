...
namespace Arcemi.Pathfinder.Kingmaker.GameData
{
    public static class JTokenExtensions
    {
        public static JToken Export(this JToken token, bool deep)
        {
            ...
            if (token.Type == JTokenType.Object)
            {
                ...
                foreach (var property in ((JObject)token).Properties())
                {
                    ...
                    obj.Add(property.Name, property.Value.Export(deep));
                }
                return obj;
            }
            return token;
        ...
        public static void ImportTo(this JToken src, JToken dest, ImportOptions options)
        {
            ...
            if (src.Type == JTokenType.Object && dest.Type == JTokenType.Object)
            {
                ...
                foreach (var srcProp in ((JObject)src).Properties())
                {
                    ...
                    {
                        srcProp.Value.ImportTo(destProp.Value, options);
                    }
                    else
                    {
        ...
        {
            return token.TryGetValue("$id", out id) && !string.IsNullOrEmpty(id);
        }
        public static bool TryGetRefId(this JToken token, out string id)
        {
            if (token.TryGetValue("$id", out id) && !string.IsNullOrEmpty(id))
                return true;
            if (token.TryGetValue("$ref", out id) && !string.IsNullOrEmpty(id))
                return true;
